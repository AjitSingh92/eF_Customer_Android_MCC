...
import com.easyfoodcustomer.adapters.AddressDialogAdapter;
import com.easyfoodcustomer.adapters.RecyclerLayoutManager;
import com.easyfoodcustomer.api.VoucherApplyInterface;
import com.easyfoodcustomer.fragments.TodayFragment;
import com.easyfoodcustomer.fragments.TommorowFragment;
...
public class TimeSlotDialogFragment extends DialogFragment implements View.OnClickListener {
    ...
    public static TimeSlotDialogFragment newInstance(Context context, OnDeliveryTimeSelectedListener onDeliveryTimeSelectedListener, String delivertyOption, boolean isCheckOut, String selectedText) {
    ...
    public void onViewCreated(@NonNull View view, @Nullable Bundle savedInstanceState) {
        ...
        if (items != null && items.size() > 0) {
            if (selectedText != null && selectedText.length() > 0) {
                tv_collection.setText(selectedText);
                for (int i = 0; i < items.size(); i++) {
                    if (items.get(i).equalsIgnoreCase(selectedText))
                        oderTypePos = i;
                }
            } else {
    ...
    public void setTimeSlot(String timeSlotttt, String isTommoroww) {
        ...
        if (isTommoroww.equalsIgnoreCase("0") && timeSlotttt.equalsIgnoreCase("ASAP")) {
            dateString = todayDate + " " + nowDate;
        } else if (isTommoroww.equalsIgnoreCase("1") && timeSlotttt.equalsIgnoreCase("ASAP")) {
            dateString = tomorrowDate + " " + nowDate;
        } else if (isTommoroww.equalsIgnoreCase("0") && !timeSlotttt.equalsIgnoreCase("ASAP")) {
    ...
    public void getTimeSlot() {
        ...
        request.setCustomer_id(sharePre.getString(sharePre.USER_ID));
        request.setServe_style(sharePre.getString(sharePre.SERVE_STYLE));

        Call<TimeSlotResponse> call3 = apiInterface.restuarantTimeSlot(PrefManager.getInstance(context).getPreference(AUTH_TOKEN, ""), request);
            ...
            public void onResponse(Call<TimeSlotResponse> call, Response<TimeSlotResponse> response) {
                ...
                try {
                    if (response.body().getSuccess()) {
                        ...
                        tomorrowDate = response.body().getTomorrowdate();
