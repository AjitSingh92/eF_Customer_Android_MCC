...
public class MenuMealDialog extends DialogFragment implements View.OnClickListener, OnMealProductItemSelect, OnProductModifierSelected, MealProductCategoryAdapter.scrollToPosition {
        ...
        switch (v.getId()) {
            ...
            case R.id.sign_up_btn_dialog:
                ...
                for (int i = 0; i < mealProductAdapters.size(); i++) {
                    if (mealProductCategoryAdapter.getCustomizableQuantity(i) > 0 && 1 > mealProductAdapters.get(i).getSelectedItem().size()) {
    ...
    public void OnSizeModifierSelected(boolean isSelect) {
        Toast.makeText(context, "" + isSelect, Toast.LENGTH_SHORT).show();
    }
}