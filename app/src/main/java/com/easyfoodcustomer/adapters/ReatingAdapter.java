...
public class ReatingAdapter extends RecyclerView.Adapter<ReatingAdapter.MyViewHolder> {
    ...
    MyViewHolder extends RecyclerView.ViewHolder {
        ...
        TextView txtName, txtDate, message;
    ...
    public void onBindViewHolder(final ReatingAdapter.MyViewHolder holder, final int listPosition) {
        ...
        if (response.getData().getReviews().size() > 0) {
            ...
            if (response.getData().getReviews().get(listPosition).getUserRating() != null && !response.getData().getReviews().get(listPosition).getUserRating().isEmpty()) {
                holder.message.setText(response.getData().getReviews().get(listPosition).getUserRating());
