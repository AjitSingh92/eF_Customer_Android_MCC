...
public abstract class OrderHistoryDao {
 ...
 @Query("SELECT * FROM OrderHistory WHERE restaurantID = :id AND isFinal=1")
    public abstract List<OrderSaveModel> loadAllOrderByMenueProductID(String id);


    @Query("SELECT * FROM OrderHistory WHERE id = :id AND isFinal=1")
    public abstract List<OrderSaveModel> loadAllOrderByID(String id);
 ...
 @Query("DELETE FROM OrderHistory WHERE id = :mealID")
    public abstract void deleteAllOrders(int mealID);
