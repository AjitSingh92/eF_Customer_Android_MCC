...
//        }
        ...
        final com.lexxdigital.easyfooduserapp.cart_model.final_cart.Datum cart2;
        if (cartList.getData().size() > 0) {

            if (cartList.getData().get(listPosition).getMenuCategory() != null) {
                Log.e("LEN CART", "Manoj Kumar");
                cart2 = (com.lexxdigital.easyfooduserapp.cart_model.final_cart.Datum) cartList.getData().get(listPosition);
                holder.listItem.setVisibility(View.VISIBLE);
